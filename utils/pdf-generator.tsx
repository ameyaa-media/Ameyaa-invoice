...
export const generatePDF = (invoice: InvoiceData) => {
  ...
  invoice.items.forEach((item) => {
    ...
    doc.text(`INR${Number(item.rate).toFixed(2)}`, 140, y);
    doc.text(`INR${item.amount.toFixed(2)}`, 160, y);
    y += 8;
  });
  ...
  doc.text(`INR${invoice.subtotal.toFixed(2)}`, 160, y);
  y += 8;
  doc.text(`Tax (${invoice.taxRate}%):`, 140, y);
  doc.text(`INR${invoice.taxAmount.toFixed(2)}`, 160, y);
  y += 8;
  doc.setFontSize(12);
  doc.text("Total:", 140, y);
  doc.text(`INR${invoice.total.toFixed(2)}`, 160, y);
